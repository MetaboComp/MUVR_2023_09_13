#' make referenc edistribution
#' @param Y the taget variable
...
H0_reference<-function(Y,
  ...
  if(is.factor(Y)|is.character(Y)|is.logical(Y)){
    ...
    if((is.factor(Y)|is.character(Y)|is.logical(Y))&(fitness=="Q2")){
  ...
  Y_new <- StatTools::sampling_from_distribution(Y) # More clever sampling ;)
...
text(mean(Ref),    ###x position of the text
